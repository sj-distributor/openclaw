...
export const GATEWAY_WINDOWS_TASK_NAME = "OMEClaw Gateway";
...
export const NODE_WINDOWS_TASK_NAME = "OMEClaw Node";
...
export function resolveGatewayWindowsTaskName(profile?: string): string {
  ...
  return `OMEClaw Gateway (${normalized})`;
}
...
export function formatGatewayServiceDescription(params?: {
  ...
  if (parts.length === 0) {
    return "OMEClaw Gateway";
  }
  return `OMEClaw Gateway (${parts.join(", ")})`;
}
...
export function formatNodeServiceDescription(params?: { version?: string }): string {
  const version = params?.version?.trim();
  if (!version) {
    return "OMEClaw Node Host";
  }
  return `OMEClaw Node Host (v${version})`;
}
